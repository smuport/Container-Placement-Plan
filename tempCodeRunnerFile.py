...
from unload_allocate import allocate
...
def sumWaitTime(task_list):
    ...
    for plan_name in task_list:
        ...
        print(f'当前{plan_name}的等待时间是{planlWaitTime}')
...
def unloadCallback(event: Event):
    ...
    if nextContainer is not None:
        ...
        # 创建 unload Event
            ...
            result = allocate(state, container)
            status='已配车'
            ...
            for area in state.block_areas:
                if area['block'] == result['block'] and area['area'] == result['area']:
                    for plan in area['plans']:
                        for result_plan in result['plans']:
                            if plan['group'] == result_plan['group']:
                                plan['capacity'] =result_plan['capacity']
...
def updateBlock_areas(result,status):
    ...
    for i in range(len(blockareas)):
        ...
        if block == result['block'] and area == result['area']:
            ...
            # 如果没有找到匹配的，添加新的instruction
            if not found:
                if instructions and instructions[0]['containerID'] == '':
                    instructions.pop(0)
